...
如果 `.venv` 中已经安装好依赖，直接在项目根目录执行：

```powershell
.\start_demo.ps1
```

脚本会同时启动 FastAPI 后端和 Streamlit 前端；如果服务已经在端口上运行，会直接复用现有服务。
...
项目已内置一份样例文档 `data/uploads/demo_power_ops.txt`，可直接在页面点击“建立索引”后体验问答和故障诊断。
